...
contract FoundationDistribution is Ownable {
    ...
    uint256 private constant _remainingDistroPercentage = 100;
    uint256 private constant _noOfRemaingDays = 360;
    uint256 private constant SECONDS_PER_DAY = 24 * 60 * 60;
    uint256 private _cliffPeriod = 180;
        ...
        returns (uint256 availablePercentage)
    {
        // x Token assigned
        // 25% on TGE
        // x - .25x tokens distributed for 270 days - 75% remaining
        // .75 * x / 270 tokens per day
        uint256 cliffDuration = _initialTimestamp + 180 days;
        uint256 vestingDuration = _initialTimestamp + 180 days + 360 days;
        ...
        if (currentTimeStamp > _initialTimestamp) {
            if (currentTimeStamp <= cliffDuration) {
                return uint256(0);
            } else if (
                currentTimeStamp > cliffDuration && currentTimeStamp < vestingDuration
            ) {
