...
describe("Cliff Distribution", function () {
    ...
    beforeEach(async function () {
        ...
        const Distribution = await ethers.getContractFactory("FoundationDistribution");
    ...
    it("checks if the funds are released after 180 days", async function () {
        ...
        const addInvestors = await distribution.addInvestors([address1.address, address2.address], [ethers.utils.parseEther("15000000"), ethers.utils.parseEther("900")])
        const withdrawableTokens = await distribution.withdrawableTokens(address1.address)
        expect(withdrawableTokens).to.equal(ethers.utils.parseEther("41666.666666666666550000"))
    });
    it("Should unlock 0 of 15000000 tokens at day 1", async function () {
        const setInitialTimestamp = await distribution.setInitialTimestamp(parseInt(Date.now() / 1000))
        const getInitalTimestamp = await distribution.getInitialTimestamp()
        ...
        await tokenEth.transfer(distribution.address, ethers.utils.parseEther("15000000"))
        await expect(
            distribution.connect(address1).withdrawTokens()
        ).to.be.revertedWith("no tokens available to withdraw.");
    });
    it("Should unlock 0 of 15000000 tokens after a month", async function () {
    ...
    it("Should unlock 0 of 15000000 tokens after 2 months", async function () {
    ...
    it("Should unlock 0 of 15000000 tokens after 3 month", async function () {
    ...
    it("Should unlock 0 of 15000000 tokens after 4 month", async function () {
    ...
    it("Should unlock 0 of 15000000 tokens after 5 month", async function () {
    ...
    it("Should unlock 0 of 15000000 tokens within 6 month", async function () {
