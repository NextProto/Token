...
describe("Duration Distribution", function () {
    ...
    beforeEach(async function () {
        ...
        const Distribution = await ethers.getContractFactory("StrategicDistribution");
    ...
    it("Should unlock 600000 of 6000000 tokens at TGE", async function () {
        const setInitialTimestamp = await distribution.setInitialTimestamp(parseInt(Date.now() / 1000))
        const getInitalTimestamp = await distribution.getInitialTimestamp()
        const addInvestors = await distribution.addInvestors([address1.address, address2.address], [ethers.utils.parseEther("3000000"), ethers.utils.parseEther("3000000")])
        await tokenEth.transfer(distribution.address, ethers.utils.parseEther("6000000"))
        await distribution.connect(address1).withdrawTokens()
        await distribution.connect(address2).withdrawTokens()
        const investor1 = await distribution.investorsInfo(address1.address)
        const investor2 = await distribution.investorsInfo(address2.address)
        expect(investor1.withdrawnTokens).to.equal(ethers.utils.parseEther("300000.00000000000"))
        expect(investor1.tokensAllotment).to.equal(ethers.utils.parseEther("3000000"))
        expect(investor2.withdrawnTokens).to.equal(ethers.utils.parseEther("300000.00000000000"))
        expect(investor2.tokensAllotment).to.equal(ethers.utils.parseEther("3000000"))
        expect((parseInt(investor1.withdrawnTokens)+parseInt(investor2.withdrawnTokens)) / (10 ** 18)).to.equal(600000)
    });
    it("Should unlock 1500000 of 6000000 tokens at 3 months from TGE", async function () {
        const setInitialTimestamp = await distribution.setInitialTimestamp(parseInt(Date.now() / 1000) - (86400 * 90))
        const getInitalTimestamp = await distribution.getInitialTimestamp()
        const addInvestors = await distribution.addInvestors([address1.address], [ethers.utils.parseEther("6000000")])
        await tokenEth.transfer(distribution.address, ethers.utils.parseEther("6000000"))
        await distribution.connect(address1).withdrawTokens()
        const investor1 = await distribution.investorsInfo(address1.address)
        expect(investor1.withdrawnTokens).to.equal(ethers.utils.parseEther("1500000.00000000000"))
    });
    it("Should unlock 2400000 of 6000000 tokens at 6 months from TGE", async function () {
        ...
        await distribution.connect(address1).withdrawTokens()
        const investor1 = await distribution.investorsInfo(address1.address)
        expect(investor1.withdrawnTokens).to.equal(ethers.utils.parseEther("2400000.00000000000"))
    });
    it("Should unlock 3300000 of 6000000 tokens at 9 months from TGE", async function () {
        ...
        const investor1 = await distribution.investorsInfo(address1.address)
        expect(investor1.exists).to.equal(true)
        expect(investor1.withdrawnTokens).to.equal(ethers.utils.parseEther("3300000.00000000000"))
        expect(investor1.tokensAllotment).to.equal(ethers.utils.parseEther("6000000"))
    });
    it("Should unlock 4200000 of 6000000 tokens at 12 months from TGE", async function () {
        ...
        await distribution.connect(address1).withdrawTokens()
        const investor1 = await distribution.investorsInfo(address1.address)
        expect(investor1.withdrawnTokens).to.equal(ethers.utils.parseEther("4200000.00000000000"))
    });
    it("Should unlock 5100000 of 6000000 tokens at 15 months from TGE", async function () {
        ...
        const investor1 = await distribution.investorsInfo(address1.address)
        const investor2 = await distribution.investorsInfo(address2.address)
        expect(investor1.withdrawnTokens).to.equal(ethers.utils.parseEther("2550000.00000000000"))
        expect(investor2.withdrawnTokens).to.equal(ethers.utils.parseEther("2550000.00000000000"))
        expect((parseInt(investor1.withdrawnTokens)+parseInt(investor2.withdrawnTokens)) / (10 ** 18)).to.equal(5100000)
        await expect(
            distribution.connect(address1).withdrawTokens()
        ).to.be.revertedWith("no tokens available to withdraw.");
    });
    it("Should unlock 6000000 of 6000000 tokens at 18 months from TGE", async function () {
        ...
        const investor1 = await distribution.investorsInfo(address1.address)
        expect(investor1.exists).to.equal(true)
        expect(investor1.withdrawnTokens).to.equal(ethers.utils.parseEther("6000000.00000000000"))
        expect(investor1.tokensAllotment).to.equal(ethers.utils.parseEther("6000000"))
    });
});