...
describe("Duration Distribution", function () {
    ...
    beforeEach(async function () {
        ...
        const Distribution = await ethers.getContractFactory("PublicDistribution");
    ...
    it("Investor Can withdraw tokens", async function () {
        ...
        expect(tokenBalanceForAdd1).to.equal(440)
    });
    ...
    it("checks if the funds are released after 1 days", async function () {
        ...
        const addInvestors = await distribution.addInvestors([address1.address, address2.address], [ethers.utils.parseEther("1100"), ethers.utils.parseEther("900")])
        const withdrawableTokens = await distribution.withdrawableTokens(address1.address)
        expect(withdrawableTokens).to.equal(ethers.utils.parseEther("440.000000000000000000"))
    })
    it("Should be able to add and fetch investors after withdrawl", async function () {
        ...
        const getInitalTimestamp = await distribution.getInitialTimestamp()
        const addInvestors = await distribution.addInvestors([address1.address, address2.address], [ethers.utils.parseEther("1100"), ethers.utils.parseEther("900")])
        await tokenEth.transfer(distribution.address, ethers.utils.parseEther("2000"))
        await distribution.connect(address1).withdrawTokens()
        await distribution.connect(address2).withdrawTokens()
        const investor1 = await distribution.investorsInfo(address1.address)
        const investor2 = await distribution.investorsInfo(address2.address)
        expect(investor1.exists).to.equal(true)
        expect(investor1.withdrawnTokens).to.equal(ethers.utils.parseEther("1100.00000000000000"))
        expect(investor1.tokensAllotment).to.equal(ethers.utils.parseEther("1100"))
        expect(investor2.exists).to.equal(true)
        expect(investor2.withdrawnTokens).to.equal(ethers.utils.parseEther("900.00000000000000"))
        expect(investor2.tokensAllotment).to.equal(ethers.utils.parseEther("900"))
        console.log("information of investors, Investor 1", `exists: ${investor1.exists}`, ` withdrawnTokens: ${investor1.withdrawnTokens} `, ` tokensAllotment: ${investor1.tokensAllotment} `)
        console.log("information of investors, Investor 2", `exists: ${investor2.exists}`, ` withdrawnTokens: ${investor2.withdrawnTokens} `, ` tokensAllotment: ${investor2.tokensAllotment} `)
    });
    it("Should unlock 800000 of 2000000 tokens at TGE", async function () {
        const setInitialTimestamp = await distribution.setInitialTimestamp(parseInt(Date.now() / 1000))
        const getInitalTimestamp = await distribution.getInitialTimestamp()
        const addInvestors = await distribution.addInvestors([address1.address, address2.address], [ethers.utils.parseEther("1000000"), ethers.utils.parseEther("1000000")])
        await tokenEth.transfer(distribution.address, ethers.utils.parseEther("6000000"))
        await distribution.connect(address1).withdrawTokens()
        await distribution.connect(address2).withdrawTokens()
        const investor1 = await distribution.investorsInfo(address1.address)
        const investor2 = await distribution.investorsInfo(address2.address)
        expect(investor1.withdrawnTokens).to.equal(ethers.utils.parseEther("400000.00000000000"))
        expect(investor1.tokensAllotment).to.equal(ethers.utils.parseEther("1000000"))
        expect(investor2.withdrawnTokens).to.equal(ethers.utils.parseEther("400000.00000000000"))
        expect(investor2.tokensAllotment).to.equal(ethers.utils.parseEther("1000000"))
    });
    it("Should unlock 1200000 of 2000000 tokens at 1 month from TGE", async function () {
        const setInitialTimestamp = await distribution.setInitialTimestamp(parseInt(Date.now() / 1000) - (86400 * 30))
        const getInitalTimestamp = await distribution.getInitialTimestamp()
        const addInvestors = await distribution.addInvestors([address1.address], [ethers.utils.parseEther("2000000")])
        await tokenEth.transfer(distribution.address, ethers.utils.parseEther("2000000"))
        await distribution.connect(address1).withdrawTokens()
        const investor1 = await distribution.investorsInfo(address1.address)
        expect(investor1.exists).to.equal(true)
        expect(investor1.withdrawnTokens).to.equal(ethers.utils.parseEther("1200000.00000000000"))
        expect(investor1.tokensAllotment).to.equal(ethers.utils.parseEther("2000000"))
    });
    it("Should unlock 1600000 of 2000000 tokens at 2 months from TGE", async function () {
    ...
    it("Should unlock 2000000 of 2000000 tokens at 3 months from TGE", async function () {
